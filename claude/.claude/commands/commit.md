...
Rules (kept inline rather than in a shared file — a `!`cat`` of a file under `~/.claude/` only resolves when cwd is the dotfiles repo, since the sandbox refuses reads outside the session's working directory):
